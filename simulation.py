...
class SingleFlySimulation(Simulation):
    ...
    def step(
        ...
        # Store visual input if available
        if "vision" in obs[key] and obs[key]["vision_updated"]:
            self.visual_inputs_hist.append(obs[key]["vision"])
        return obs[key], reward, terminated, truncated, info[key]
    
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        self.visual_inputs_hist = []  # Clear vision history on reset
        obs, info = super().reset(seed=seed, options=options)  # Call parent's reset
        return obs[self.fly.name], info[self.fly.name]  # Return fly-specific obs/info
